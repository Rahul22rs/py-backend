...
from app.repo.CisCaseDetailRepo import CisCaseDetailRepo
from app.request.CisCaseDetailRequest import CisCaseDetailRequest
from app.utils.JwtUtils import JwtUtils
import logging
jwtUtils = JwtUtils()


class CisCaseDetailRouter:
    def __init__(self):
        logging.basicConfig(
            level=logging.INFO,
            filename="app.log",
        ...
        self.router = APIRouter(prefix="/case_detail")
        self.repo = CisCaseDetailRepo()
        self.logger = logging.getLogger("CisCaseDetailRouter")
    ...
    async def create(self, request: CisCaseDetailRequest, response: Response):
        try:
            print(request)
            self.logger.info(f"------- create : {request}")
            payload = (
    ...
    async def update(self, id: int, request: CisCaseDetailRequest):
        record = self.repo.find(id)
        if record is not None:
