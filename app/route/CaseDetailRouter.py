...
from app.repo.CaseDetailRepo import CaseDetailRepo
from app.request.CaseDetailRequest import CaseDetailRequest
from app.utils.JwtUtils import JwtUtils
import logging
jwtUtils = JwtUtils()


class CaseDetailRouter:
    def __init__(self):
        logging.basicConfig(
            level=logging.INFO,
            filename="app.log",
    ...
    async def create(self, request: CaseDetailRequest, response: Response):
        try:
            print(request)
    ...
    async def update(self, id: int, request: CaseDetailRequest):
        record = self.repo.find(id)
        if record is not None:
